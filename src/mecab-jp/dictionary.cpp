...
int calcCost(const std::string &w, const std::string &feature,
  ...
  LearnerNode rnode;
  LearnerNode lnode;
  rnode.stat  = lnode.stat = MECAB_JP_NOR_NODE;
  rnode.rpath = &path;
  lnode.lpath = &path;
  path.lnode  = &lnode;
  path.rnode  = &rnode;
...
// Define the types manually instead of inheriting
template <typename T1, typename T2>
struct pair_1st_cmp {
  typedef std::pair<std::string, MeCabJp::Token*> first_argument_type;
  typedef std::pair<std::string, MeCabJp::Token*> second_argument_type;
...
bool Dictionary::compile(const Param &param,
  ...
  for (size_t i = 0; i < dics.size(); ++i) {
    ...
    while (is->getline(line.get(), line.size())) {
      ...
      if (cost == INT_MAX) {
        CHECK_DIE(type == MECAB_JP_USR_DIC)
      ...
      if (type != MECAB_JP_UNK_DIC && !iconv.convert(&w)) {
        std::cerr << "iconv conversion failed. skip this entry"
                  << std::endl;
        continue;
      }
