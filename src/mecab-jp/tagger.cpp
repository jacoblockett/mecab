...
const MeCabJp::Option long_options[] = {
...
ModelImpl::ModelImpl()
    : viterbi_(new Viterbi), writer_(new Writer),
      request_type_(MECAB_JP_ONE_BEST), theta_(0.0) {}
...
bool ModelImpl::swap(Model *model) {
  ...
  if (!is_available()) {
    setGlobalErrorJp("current model is not available");
    return false;
  }
#ifndef HAVE_ATOMIC_OPS
  setGlobalErrorJp("atomic model replacement is not supported");
  return false;
#else
  ModelImpl *m = static_cast<ModelImpl *>(model_data.get());
  if (!m) {
    setGlobalErrorJp("Invalid model is passed");
    return false;
  }

  if (!m->is_available()) {
    setGlobalErrorJp("Passed model is not available");
    return false;
  }
...
Tagger *ModelImpl::createTagger() const {
  if (!is_available()) {
    setGlobalErrorJp("Model is not available");
    return 0;
  }
  TaggerImpl *tagger = new TaggerImpl;
  if (!tagger->open(*this)) {
    setGlobalErrorJp(tagger->what());
    delete tagger;
    return 0;
  }
...
TaggerImpl::TaggerImpl()
    : current_model_(0),
      request_type_(MECAB_JP_ONE_BEST), theta_(kDefaultTheta) {}
...
void TaggerImpl::set_partial(bool partial) {
  if (partial) {
    request_type_ |= MECAB_JP_PARTIAL;
  } else {
    request_type_ &= ~MECAB_JP_PARTIAL;
  }
}
...
bool TaggerImpl::partial() const {
  return request_type_ & MECAB_JP_PARTIAL;
}
...
void TaggerImpl::set_lattice_level(int level) {
  switch (level) {
    case 0: request_type_ |= MECAB_JP_ONE_BEST;
      break;
    case 1: request_type_ |= MECAB_JP_NBEST;
      break;
    case 2: request_type_ |= MECAB_JP_MARGINAL_PROB;
      break;
    default:
      break;
  }
...
int TaggerImpl::lattice_level() const {
  if (request_type_ & MECAB_JP_MARGINAL_PROB) {
    return 2;
  } else if (request_type_ & MECAB_JP_NBEST) {
    return 1;
  } else {
    return 0;
  }
...
void TaggerImpl::set_all_morphs(bool all_morphs) {
  if (all_morphs) {
    request_type_ |= MECAB_JP_ALL_MORPHS;
  } else {
    request_type_ &= ~MECAB_JP_ALL_MORPHS;
  }
}
...
bool TaggerImpl::all_morphs() const {
  return request_type_ & MECAB_JP_ALL_MORPHS;
}
...
bool TaggerImpl::parseNBestInit(const char *str, size_t len) {
  Lattice *lattice = mutable_lattice();
  initRequestType();
  lattice->add_request_type(MECAB_JP_NBEST);
  lattice->set_sentence(str, len);
  if (!parse(lattice)) {
    set_what(lattice->what());
    return false;
...
LatticeImpl::LatticeImpl(const Writer *writer)
    : sentence_(0), size_(0), theta_(kDefaultTheta), Z_(0.0),
      request_type_(MECAB_JP_ONE_BEST),
      writer_(writer),
      ostrs_(0),
...
void LatticeImpl::set_sentence(const char *sentence, size_t len) {
  ...
  if (has_request_type(MECAB_JP_ALLOCATE_SENTENCE) ||
      has_request_type(MECAB_JP_PARTIAL)) {
...
bool LatticeImpl::next() {
  if (!has_request_type(MECAB_JP_NBEST)) {
    set_what("MECAB_JP_NBEST request type is not set");
    return false;
  }
...
const char *LatticeImpl::enumNBestAsStringInternal(size_t N,
  ...
  if (writer_) {
    ...
    eon_node.stat = MECAB_JP_EON_NODE;
    eon_node.next = 0;
...
void LatticeImpl::set_boundary_constraint(size_t pos,
                                          int boundary_constraint_type) {
  if (boundary_constraint_.empty()) {
    boundary_constraint_.resize(size() + 4, MECAB_JP_ANY_BOUNDARY);
  }
...
void LatticeImpl::set_feature_constraint(size_t begin_pos, size_t end_pos,
  ...
  set_boundary_constraint(begin_pos, MECAB_JP_TOKEN_BOUNDARY);
  set_boundary_constraint(end_pos, MECAB_JP_TOKEN_BOUNDARY);
  for (size_t i = begin_pos + 1; i < end_pos; ++i) {
    set_boundary_constraint(i, MECAB_JP_INSIDE_TOKEN);
  }
...
int mecab_jp_do(int argc, char **argv) {
#define WHAT_ERROR(msg) do {                    \
  ...
  MeCabJp::Param param;
  if (!param.open(argc, argv, MeCabJp::long_options)) {
  ...
  MeCabJp::scoped_ptr<MeCabJp::ModelImpl> model(new MeCabJp::ModelImpl);
  if (!model->open(param)) {
    std::cout << MeCabJp::getLastError() << std::endl;
    return EXIT_FAILURE;
  }
  ...
  MeCabJp::ostream_wrapper ofs(ofilename.c_str());
  if (!*ofs) {
  ...
  if (param.get<bool>("dictionary-info")) {
    for (const MeCabJp::DictionaryInfo *d = model->dictionary_info();
         d; d = d->next) {
  ...
  MeCabJp::scoped_array<char> ibuf_data(new char[ibufsize]);
  char *ibuf = ibuf_data.get();

  MeCabJp::scoped_ptr<MeCabJp::Tagger> tagger(model->createTagger());
  ...
  for (size_t i = 0; i < rest.size(); ++i) {
    MeCabJp::istream_wrapper ifs(rest[i].c_str());
    if (!*ifs) {
    ...
    while (true) {
      if (!partial) {
        ...
        std::string sentence;
        MeCabJp::scoped_fixed_array<char, BUF_SIZE> line;
        for (;;) {
