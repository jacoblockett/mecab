...
#ifndef MECAB_JP_LEARNER_NODE_H_
#define MECAB_JP_LEARNER_NODE_H_
...
struct mecab_jp_learner_path_t {
  struct mecab_jp_learner_node_t*  rnode;
  struct mecab_jp_learner_path_t*  rnext;
  struct mecab_jp_learner_node_t*  lnode;
  struct mecab_jp_learner_path_t*  lnext;
  double                        cost;
  const int                     *fvector;
};

struct mecab_jp_learner_node_t {
  struct mecab_jp_learner_node_t *prev;
  struct mecab_jp_learner_node_t *next;
  struct mecab_jp_learner_node_t *enext;
  struct mecab_jp_learner_node_t *bnext;
  struct mecab_jp_learner_path_t *rpath;
  struct mecab_jp_learner_path_t *lpath;
  struct mecab_jp_learner_node_t *anext;
  const char                  *surface;
  const char                  *feature;
  unsigned int                 id;
  ...
  double                       cost;
  const int                    *fvector;
  struct mecab_jp_token_t         *token;
};

namespace MeCabJp {

typedef struct mecab_jp_learner_path_t LearnerPath;
typedef struct mecab_jp_learner_node_t LearnerNode;
...
inline bool node_cmp_eq(const LearnerNode &node1,
  ...
  if (node1.length == node2.length &&
    ...
    // There is NO case when node1 becomes MECAB_JP_UNK_NODE
    if (node2.stat == MECAB_JP_UNK_NODE)
...
inline bool is_empty(LearnerPath *path) {
  return ((!path->rnode->rpath && path->rnode->stat != MECAB_JP_EOS_NODE) ||
          (!path->lnode->lpath && path->lnode->stat != MECAB_JP_BOS_NODE) );
}
...
inline void calc_expectation(LearnerPath *path, double *expected, double Z) {
  ...
  if (path->rnode->stat != MECAB_JP_EOS_NODE) {
...
#endif  // MECAB_JP_LEARNER_NODE_H_
