...
class DictionaryComplier {
 public:
  static int run(int argc, char **argv) {
    static const MeCabJp::Option long_options[] = {
      ...
      { "charset",   'c',  MECAB_JP_DEFAULT_CHARSET, "ENC",
        "make charset of binary dictionary ENC (default "
        MECAB_JP_DEFAULT_CHARSET ")"  },
      { "charset",   't',  MECAB_JP_DEFAULT_CHARSET, "ENC", "alias of -c"  },
      { "dictionary-charset",  'f',  MECAB_JP_DEFAULT_CHARSET,
...
#define OCONF(file) create_filename(outdir, std::string(file)).c_str()
    ...
    if (!userdic.empty()) {
      ...
      param.set("type", static_cast<int>(MECAB_JP_USR_DIC));
      ...
      if (opt_unknown) {
        ...
        param.set("type", static_cast<int>(MECAB_JP_UNK_DIC));
      ...
      if (opt_sysdic) {
        CHECK_DIE(dic.size()) << "no dictionaries are specified";
        param.set("type", static_cast<int>(MECAB_JP_SYS_DIC));
...
int mecab_jp_dict_index(int argc, char **argv) {
  return MeCabJp::DictionaryComplier::run(argc, argv);
}
