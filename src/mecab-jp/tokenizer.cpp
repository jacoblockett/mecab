...
bool is_valid_node(const Lattice *lattice,  N *node) {
  ...
  if (lattice->boundary_constraint(end_pos) == MECAB_JP_INSIDE_TOKEN) {
    return false;
  }
  ...
  if (lattice->boundary_constraint(begin_pos) == MECAB_JP_TOKEN_BOUNDARY &&
      lattice->boundary_constraint(end_pos) == MECAB_JP_TOKEN_BOUNDARY &&
...
#define ADDUNKNWON do {                                                  \
    ...
    for (size_t k = 0; k < size; ++k) {                                  \
      ...
      new_node->stat = MECAB_JP_UNK_NODE;                                   \
...
N *Tokenizer<N, P>::lookup(const char *begin, const char *end,
  ...
  if (isPartial) {
    ...
    for (size_t n = begin_pos + 1; n < lattice->size(); ++n) {
      if (lattice->boundary_constraint(n) == MECAB_JP_TOKEN_BOUNDARY) {
  ...
  for (std::vector<Dictionary *>::const_iterator it = dic_.begin();
    ...
    for (size_t i = 0; i < n; ++i) {
      ...
      for (size_t j = 0; j < size; ++j) {
        ...
        new_node->surface = begin2;
        new_node->stat = MECAB_JP_NOR_NODE;
        new_node->char_type = cinfo.default_type;
