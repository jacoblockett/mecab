...
class Eval {
 ...
 public:
  static bool eval(int argc, char **argv) {
    ...
    MeCabJp::ostream_wrapper ofs(output.c_str());
 ...
 public:
  static int run(int argc, char **argv) {
    ...
    for (size_t i = 0; i < files.size(); ++i) {
      MeCabJp::istream_wrapper ifs(files[i].c_str());
...
// exports
int mecab_jp_system_eval(int argc, char **argv) {
  return MeCabJp::Eval::eval(argc, argv);
}

int mecab_jp_test_gen(int argc, char **argv) {
  return MeCabJp::TestSentenceGenerator::run(argc, argv);
}
