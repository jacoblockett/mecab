...
bool Viterbi::analyze(Lattice *lattice) const {
  ...
  bool result = false;
  if (lattice->has_request_type(MECAB_JP_NBEST) ||
      lattice->has_request_type(MECAB_JP_MARGINAL_PROB)) {
    // IsAllPath=true
...
bool Viterbi::forwardbackward(Lattice *lattice) {
  if (!lattice->has_request_type(MECAB_JP_MARGINAL_PROB)) {
    return true;
  }
...
bool Viterbi::buildAllLattice(Lattice *lattice) {
  if (!lattice->has_request_type(MECAB_JP_ALL_MORPHS)) {
    return true;
  }
...
bool Viterbi::buildAlternative(Lattice *lattice) {
  ...
  for (const Node *node = bos_node; node; node = node->next) {
    if (node->stat == MECAB_JP_BOS_NODE || node->stat == MECAB_JP_EOS_NODE) {
      continue;
    }
...
bool Viterbi::initPartial(Lattice *lattice) {
  if (!lattice->has_request_type(MECAB_JP_PARTIAL)) {
    if (lattice->has_constraint()) {
      lattice->set_boundary_constraint(0, MECAB_JP_TOKEN_BOUNDARY);
      lattice->set_boundary_constraint(lattice->size(),
                                       MECAB_JP_TOKEN_BOUNDARY);
    }
    return true;
  }
  ...
  for (size_t i = 0; i < tokens.size(); ++i) {
    ...
    lattice->set_boundary_constraint(pos, MECAB_JP_TOKEN_BOUNDARY);
    lattice->set_boundary_constraint(pos + len, MECAB_JP_TOKEN_BOUNDARY);
    if (feature) {
      lattice->set_feature_constraint(pos, pos + len, feature);
      for (size_t n = 1; n < len; ++n) {
        lattice->set_boundary_constraint(pos + n,
                                         MECAB_JP_INSIDE_TOKEN);
      }
    }
    pos += len;
  }
