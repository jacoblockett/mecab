...
const char *getGlobalErrorKo();
void setGlobalErrorKo(const char *str);
...
bool ModelImpl::swap(Model *model) {
  ...
  if (!is_available()) {
    setGlobalErrorKo("current model is not available");
    return false;
  }
#ifndef HAVE_ATOMIC_OPS
  setGlobalErrorKo("atomic model replacement is not supported");
  return false;
#else
  ModelImpl *m = static_cast<ModelImpl *>(model_data.get());
  if (!m) {
    setGlobalErrorKo("Invalid model is passed");
    return false;
  }

  if (!m->is_available()) {
    setGlobalErrorKo("Passed model is not available");
    return false;
  }
...
Tagger *ModelImpl::createTagger() const {
  if (!is_available()) {
    setGlobalErrorKo("Model is not available");
    return 0;
  }
  TaggerImpl *tagger = new TaggerImpl;
  if (!tagger->open(*this)) {
    setGlobalErrorKo(tagger->what());
    delete tagger;
    return 0;
  }
